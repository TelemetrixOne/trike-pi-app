...
class RaceContractTests(unittest.TestCase):
    ...
    def test_hdmi_compositor_uses_direct_camera_inputs(self) -> None:
        ...
        self.assertIn('resolve_cameras', source)
        ...
        self.assertIn('HDMI-priority mode', source)
        self.assertIn('Streaming-priority mode', source)
        self.assertIn('Camera or HDMI state changed; rebuilding video mode.', source)
        self.assertNotIn('Waiting for two distinct USB cameras', source)
    ...
    def test_video_publishers_do_not_wait_for_hdmi(self) -> None:
        ...
        self.assertIn('hdmi_connected()', source)
        self.assertIn('if hdmi_connected; then', source)
        self.assertIn('Streaming-priority mode', source)
